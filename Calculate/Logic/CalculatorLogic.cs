...
namespace Calculator.Logic
{
    public class CalculatorLogic
    {
        ...
        public CalculatorState SaveState()
        {
            return new CalculatorState
            {
                ...
                IsNewEntry = _isNewEntry
            };
        }

        public void RestoreState(CalculatorState state)
        {
            _firstNumber = state.FirstNumber;
            _secondNumber = state.SecondNumber;
            _currentOperation = state.CurrentOperation;
            _currentInput = state.CurrentInput;
            _isNewEntry = state.IsNewEntry;

            // Обновляем дисплей после восстановления состояния
